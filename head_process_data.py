...
def read_dataset(s_path, t_path):
    ...
    while True:
        ...
        if len(s_dict[user]) > max_u:
            max_u = len(s_dict[user])
        if len(s_dict[item]) > max_i:
            max_i = len(s_dict[item])
    ...
    while True:
        ...
        if len(t_dict[user]) > max_u:
            max_u = len(t_dict[user])
        if len(t_dict[item]) > max_i:
            max_i = len(t_dict[item])
    ...
    f = open('../meta_learning/data/poincare_glove_100D.txt')
    ...
    return s_data, s_dict, t_train, t_valid, t_test, t_dict, w_embed, [max_u, max_i]
